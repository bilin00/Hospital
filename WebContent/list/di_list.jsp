...
<%@page import="com.myh.bean.disease"%>
...
<table style="width:800px; margin:44px auto" class="table table-striped table-bordered table-hover  table-condensed" align='center' border='1' cellspacing='0'>
		...
		<tr bgcolor="#EECBAD">
			<td align="center" colspan="9">
				<h2>Data of the diseases</h2>
			</td>
		</tr>

<tr align="center">
			<td title="Disease's ID Number"><b>DEID</b></td>
			<td title="Disease's Name"><b>DENAME</b></td>
			<td><b>Edit</b></td>
			<td><b>Delete</b></td>
		</tr>

		<%
			List<disease> dis = (List<disease>) request.getAttribute("dis");
			if (dis== null || dis.size() < 1) {
				out.print("No data!");
			} 
		%>

    <c:forEach items="${dis}" var="di" varStatus="st">
        <tr align="center"  bgcolor="#FFF8DC">
            <td>${di.getDeid()}</td>
            <td>${di.getDename()}</td>
            <td><a href="edit/di_edit.jsp?deid=${di.getDeid()}&dename=${di.getDename()}&start=${start}&sql_option=${sql_option}&val=${val}">edit</a></td>
            <td><a href="DiDelete?id=${di.getDeid()}&start=${start}&sql_option=${sql_option}&val=${val}">delete</a></td>
        </tr>
    </c:forEach>
</table>

<form class="form-signin" action="/DiAdd?main=yes&sql_option=${sql_option}" onsubmit="check(this)" name="input" method="post">
<table style="width:800px; margin:44px auto" class="table table-striped table-bordered table-hover  table-condensed" align='center' border='1' cellspacing='0'>
...
<tr align="center">
                				...
                				<td><input type="text" id="deid" name="deid" class="form-control" onkeyup="value=value.replace(/[^\d]/g,'')" maxlength="6" placeholder="Please type in deid" required autofocus></td>
                				<td><input type="text" id="dename" name="dename" class="form-control" onkeyup="this.value=this.value.toUpperCase()" placeholder="Please type in dename" required></td>
</tr>	
</table>
</form>
...
<form class="form-horizontal has-success" action="DiList" role=form method="post">    
...
<div class="input-group-btn">
                        <select id="select" name="type" class="form-control" style="width: auto">
                            ...
                            <option value="dename">dname</option>
                        </select>
</div>
</td>
<td align="center" colspan="8">
                    <input id="inputSearch" type="text" name="deid" class="form-control input-sm" placeholder="Search for ..." required>
</td>
